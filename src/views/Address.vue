  ...
  <div>
      ...
      <div class="container">
        <div class="checkout-addr">
            ...
            <div class="addr-list">
              <ul>
                <li v-for="(item,index) in addressListFilter " v-bind:class="{'check':checkIndex==index}"
                  ...
                  <dl>
                    <dt>{{item.userName}}</dt>
                    <dd class="address">{{item.phoneNum}}</dd>
                    <dd class="tel">{{item.email}}</dd>
                  </dl>
                ...
                <li class="addr-new" href="javascript:;" @click="addAddressFlag=true">
          ...
          <div class="lemall-msg-info hidden">
            <span>您选择的区域不在我们的交付区域内。请在送货区域内选择另一个送货地址。</span>
          </div>
    ...
    <div class="  md-modal modal-msg md-modal-transition" v-bind:class="{'md-show':addAddressFlag}">
      <div class="md-modal-inner">
        <div class="md-top">
          <div class="md-title title-text">填写手机号和邮箱</div>
          <button class="md-close" @click="addAddressFlag=false">关闭</button>
        </div>
        <div class="md-content">
          <div class="confirm-tips content-767">
            <div class="error-wrap">
              <span class="error error-show" v-show="true">请仔细填写手机和邮箱</span>
            </div>
            <div class="error-wrap">
              <span class="error error-show" v-show="errorTip">姓名、手机和邮箱不能为空</span>
            </div>
            <ul class="md-ul">
              <li class="regi_form_input">
                <i class="icon IconPeople"></i>
                <input type="text" tabindex="1" name="userName" v-model="userName"
                       class="regi_login_input regi_login_input_left" placeholder="收件人姓名" data-type="userName">
              </li>
              <li class="regi_form_input">
                <i class="icon IconPeople" style="background-position: -100px 5px;!important"></i>
                <input type="text" tabindex="1" name="phoneNum" v-model="phoneNum"
                       class="regi_login_input regi_login_input_left" placeholder="手机号码" data-type="phoneNum">
              </li>
              <li class="regi_form_input noMargin">
                <i class="icon IconPwd" style="background-position: -500px 5px;!important"></i>
                <input type="text" tabindex="2" name="email" v-model="email"
                       class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="电子邮箱">
              </li>
            </ul>
          </div>
    ...
    <div class="md-overlay" v-if="addAddressFlag" @click="addAddressFlag"></div>
  </div>
</template>
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      addAddress(){
        if (!this.phoneNum || !this.email || !this.userName) {
          //用户密码为空是报错
          this.errorTip = true;
          return;
        } else {
        ...
        axios.post("/users/addAddress", {
          ...
          if (res.status == '0') {
            this.errorTip = false;
            this.addAddressFlag = false;//关闭弹框
            this.init();
            //todo
          }
        })
