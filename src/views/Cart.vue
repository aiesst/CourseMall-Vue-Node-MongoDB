        ...
        <div class="cart-foot-wrap">
          <div class="cart-foot-inner">
            ...
            <div class="cart-foot-r">
              ...
              <div class="btn-wrap">
                <a class="btn btn--red" v-bind:class="{'btn--dis':checkedCount==0}" @click="checkOut">购物车结算</a>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default{
    ...
    computed: {//实时处理计算
      checkAllFlag(){
        //如果选中的条数是购物车的条数，那么selectAll 选中标志
        return this.checkedCount == this.cartList.length;
      },
      checkedCount(){//计算属性而非方法
        var i = 0;
    ...
    methods: {
      ...
      delCart(){
        axios.post("/users/cartDel", {
          productId: this.delItem.productId,//点击的商品ID
          ...
          if (res.status == '0') {
            ...
            this.init();//刷新页面数据
            console.log(this.delItem.productNum);
      ...
      editCart(flag, item){
        if (flag == "add") {
          ...
          this.$store.commit("updateCartCount", 1);//更新购物车标志
        } else if (flag == "minu") {
